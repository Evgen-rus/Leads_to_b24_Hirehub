...
- 1_save_gsheet_to_sqlite.py читает Google Sheets и сохраняет лиды в `lr195.db`
- 2_upload_sqlite_to_bitrix.py берет из `lr195.db` только лиды с пустым `Статус_Б24`, создает лиды в Bitrix24 и записывает ссылку обратно в Google Sheets и SQLite
